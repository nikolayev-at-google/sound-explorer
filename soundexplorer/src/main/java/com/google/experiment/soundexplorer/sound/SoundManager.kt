...
class SoundManager : Closeable {
    ...
    fun loadSound(inputStream: InputStream, session: Session, entity: Entity): Int? {
        ...
        try {
            ...
            val riffId = String(inputStream.readNBytes(4)) // String(headerBuffer, 0, 4)
            if (riffId != "RIFF") throw IOException("Invalid WAV file - Missing 'RIFF'")
            val waveId = String(inputStream.readNBytes(8), 4, 4) // headerBuffer, 8, 4)
            if (waveId != "WAVE") throw IOException("Invalid WAV file - Missing 'WAVE'")

            // todo - consider adding additional wav header validation checks
            ...
            val audioAttributes = AudioAttributes.Builder()
                .setUsage(AudioAttributes.USAGE_ASSISTANCE_SONIFICATION) // MEDIA ?
                .setContentType(AudioAttributes.CONTENT_TYPE_SONIFICATION) // MUSIC ?
                .build()
    ...
    fun playAllSounds() {
        ...
        for (i in 1..<this.audioTracks.size) {
            // todo - AudioTrack exposes the ability to manipulate the playhead position on an audiotrack
            // instance that is stopped or paused. However, getPlaybackHeadPosition() returns 0 after setting
            // it for some reason. Experiment further with manipulating the playhead to account for lag.

            // val ctFrames = ((System.nanoTime() - st) * 0.000000001) * this.audioTracks[i].sampleRate
            // val result = this.audioTracks[i].setPlaybackHeadPosition(ctFrames.toInt())
